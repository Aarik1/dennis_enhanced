...
020 eab	eab 7/10/0	BATS 982 gettar re-written to better support unified phonemes 
...
short us_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
	...
	{
		//BATS 982 gettar re-written to better support unified phonemes eab 7/10/01
	...
	else if (pDphsettar->par_type IS_AV_OR_AH)
	{
		...
		else
		{
			if ((phone_temp) == USP_HX)
			{
				...
				if (begtyp(phnex_temp)  != 1)
				{
					tartemp = 56;	   /* Stronger asp before +back */
				}
			}
			else
			{
...
//Set it once per loop whne PAB
			...
			{
			if(us_place[pDphsettar->phcur & PVALUE] & FLABIAL)
			{
		 ...
		 && ((feacur & FSONOR) IS_PLUS ))) 
	{
		...
		//eab this look \s like a mistake in that we're really already at the sonorant and we should 
		//be lookin at it
		//help 
		//if (begtyp(pDphsettar->phonex) != 1)
		if (begtyp(pDphsettar->phcur) != 1)
		{
			PAP.pspesh = 55;		   /* Stronger asp before +back */
		}
		PAV.pspesh = 0;				   /* Amplitude of voicing during aspir */


		vot = NF50MS;				   /* Asp dur for /p,t,k/ before stressed sonor */
		//esb ref 1001 adding in word inital position
		//VOT FOR K SHOULD BE LONGER P=50, T=60 K=65
		//P A LITTLE SHORTER K A LITTLE LONGER 
	//	if ((struclas & FWINITC) IS_MINUS)
	//		vot = 2; //2 frames
		//change to syl final
		...
		{
			//help this looks too short for female at lest
#if (defined  FAKE_HLSYN || !(defined HLSYN) )
			vot = NF7MS;			   /* Vot shorter if vowel not stressed */
#else
			vot = NF7MS;
#endif
		...
		if (  pDph_t->allophons[pDph_t->nphone -2]== USP_S)          
		{
			/* Plosive is in an [s] cluster */
			if ((pDph_t->allofeats[pDph_t->nphone - 2] & FBOUNDARY) IS_MINUS )
			{
				vot = NF15MS;	   
			}
		}

...
short la_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
	...
	if (pDphsettar->np < &PFZ)
		pphotr = (npar) * LA_TOT_ALLOPHONES;
	else
		pphotr = (npar - 1) * LA_TOT_ALLOPHONES;
	...
	if ((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW)
	{
		...
		if ((npar == (B3 - F1))
			&& (phone_temp == LAP_N
				|| phone_temp == LAP_NH
				|| phone_temp == LAP_NX)
			&& (((la_place[phnex_temp & PVALUE] & F2BACKI) IS_PLUS)
				|| ((la_place[phlas_temp & PVALUE] & F2BACKI) IS_PLUS)
			)
			)
		{
		...
		/* 
	 	 ...
	 	 */
		if ((npar == (B3 - F1))
			&& ((la_place[phone_temp & PVALUE] & F2BACKI) IS_PLUS)
			&& (phlas_temp == LAP_F))
		{
			tartemp = 90;
		}
		/* 
	 	 ...
	 	 */
		if ((phone_temp == LAP_R || phone_temp == LAP_RR)
			&& (phlas_temp == LAP_O || phlas_temp == LAP_U))
		{
			switch (npar)
			{
				case (F1 - F1):
	...
	else if (pDphsettar->par_type IS_PARALLEL_FORM_AMP)
	{
		...
		if (pDphsettar->np == &PTILT)
		{
			...
			else if (phone_temp == LAP_L && (phnex_temp == LAP_U || phlas_temp == LAP_U))
			{
				tartemp = 10 - 5 ;
			}*/
    		else if ((phone_feat & (FVOICD | FPLOSV)) == (FVOICD | FPLOSV)
					 && ((phone_temp != LAP_DH) || (phone_temp == LAP_YH)))
			{
				tartemp = 12;
			}
			 ...
			 */
			else if (phone_temp == LAP_R || phone_temp == LAP_RR)
			{
				tartemp = 24;
			}
			/* 
			 ...
			 */
		//	else if ((phone_temp >= LAP_BH)
			// trying to use out_gs for tilt 
			else if ((phone_temp >= LAP_DH)
					 && (phone_temp <= LAP_GH))
			{
				tartemp = 24;
			}
 ...
 *
 * *****************************************************************/
static short la_special_coartic(PDPH_T pDph_t, short nfon, short diphpos)
{
...

{
	...
	else if (pDphsettar->np == &PF2)
	{
		...
		case LAP_E:
			...
			case LAP_I :		   /* eab 7/16/98* more code for double vowel dipthong*/
	...
	else if (pDphsettar->par_type IS_FORM_FREQ)
	{
		...
		if ((feacur & FSONOR) IS_PLUS)
		{
			if ((feacur & FSONCON) IS_MINUS)
			{
				...
				if (pDphsettar->phcur == LAP_YH)	   /* Faster for y */
				...
				if ((fealas & FSONCON) IS_PLUS)
				{
					...
					if (pDphsettar->phcur != GEN_SIL
						&& (pholas == LAP_L) && (pDphsettar->np == &PF1))
					{
						pDphsettar->bouval += 80;
					}
#if 0
					/* Make F3 & F2 transitions slower out of /r/ */
					if (((pholas == LAP_R) || (pholas == LAP_RR))
						&& (pDphsettar->np != &PF1))
					{
				...
				{
					/* 2. Vowel-[vowel/dh,gh,bh,LAP_J] transition */
					/* Preceeding seg has strong influence on [h] init val */
					/* if (phonex == LAP_DH) { bouval = (bouval + np->tarlas) >> 1;
					 * 
 					 * }            
 					 */
...
/* OUT                  xx += ((bouval - np->tarend) >> 1); np->tarend += xx;   (DOESN'T WORK) END OUT */
			...
			if ((feacur & FNASAL) IS_PLUS)
			{
				...
				else if (((pDphsettar->phcur == LAP_N))
						 && (begtyp(pDphsettar->phonex) == 1))
				{
					if (pDphsettar->np == &PF2)
					{
						pDphsettar->bouval -= 100;
						if ((la_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS)
						{
							pDphsettar->bouval -= 100;
						}
					}
				...
				/* Lower F2 of [m] nasal murmur near [i,e,ix] */
						 ...
						 && (pDphsettar->phcur == LAP_M
							 || pDphsettar->phcur == LAP_MX)
						 && ((la_place[pholas & PVALUE] & F2BACKF) IS_PLUS))
				{
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		if ((pDphsettar->np == &PB3)
			&& ((la_place[pDphsettar->phcur & PVALUE] & F2BACKI) IS_PLUS)
			&& (pholas == LAP_F))
		{
			pDphsettar->durtran = 0;
			 ...
			 || (pDphsettar->par_type IS_AV_OR_AH))
	{
		...
		{
			if ((pDphsettar->phcur == LAP_CH)
				|| (pDphsettar->phcur == LAP_YH))
			{
	...
	else if (pDphsettar->par_type IS_NASAL_ZERO_FREQ)
	{
			...
			&& ((feacur & FNASAL) IS_MINUS))
		{
			 ...
			 */
			pDphsettar->durtran = (pholas == LAP_NH) ? NF80MS : NF40MS;
		}
	}
...
/* *****************************************************************/
			...
			if ((feacur & FSONCON) IS_MINUS)
			{
				if ((feanex & FSONCON) IS_PLUS)
				{
					...
					if (pDphsettar->phcur != GEN_SIL
						&& (pDphsettar->phonex == LAP_L) && (pDphsettar->np == &PF1))
					{
						pDphsettar->bouval += 80;
					}
				}
		...
		else
		{
			...
			if ((feacur & FNASAL) IS_PLUS)
			{
				...
				/* Lower F2 of [m] murmur near [i,y,yx] */
						 ...
						 && (pDphsettar->phcur == LAP_M || pDphsettar->phcur == LAP_MX)
						 && ((la_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS))
				{
					pDphsettar->bouval -= 150;
				}
#endif
			 ...
			 || (pDphsettar->par_type IS_AV_OR_AH))
	{
...
#else
#ifdef LAP_V
			&& pDphsettar->phcur != LAP_V
#endif
			&& pDphsettar->phcur != LAP_YH   /* 7-May-86    */
			&& pDphsettar->phcur != LAP_Z)
		{
			/* 
			 * All but Y, V, Z
			 */
			...
			if (((feacur & FPLOSV) IS_PLUS)
				|| (pDphsettar->phcur == LAP_CH)
				|| (pDphsettar->phcur == LAP_YH)
				|| (pDphsettar->phcur == LAP_GH))
			{
				if ((feacur & FPLOSV) IS_PLUS)
				{
					if ((feacur & FVOICD) IS_PLUS)
		 ...
		 */
		if ((pDphsettar->np == &PAV)
			&& (pDphsettar->phcur == LAP_S || pDphsettar->phcur == LAP_TH)
			&& ((feanex & (FCONSON | FVOICD)) == (FCONSON | FVOICD)))
		{
			pDphsettar->durtran = pDph_t->durfon >> 1;
			//debugbackward (" [s, th] prevoicing rule",pDphsettar->np);
		}
		if (pDphsettar->phcur == LAP_TH && pDph_t->th_to_s == TRUE)
		{
			pDphsettar->phcur = LAP_S;
		}
		...
		if ((feacur & FPLOSV) IS_PLUS)
		{
			...
			if (pDphsettar->phcur != LAP_CH && pDphsettar->phcur != LAP_YH)
			{
...
/* *****************************************************************/
		...
		if (((feanex & FPLOSV) IS_PLUS)
			|| (pDphsettar->phonex == LAP_YH)
			|| (pDphsettar->phonex == LAP_CH))
		{
			pDphsettar->durtran = 0;
...
//	PKSD_T                  pKsd_t = phTTS->pKernelShareData;
	...
	if ((feacur & FBURST) IS_PLUS)
	{
		...
		else
		{
			...
			if (((feanex & (FNASAL | FPLOSV)) != 0)
				&& la_place[pDphsettar->phcur & PVALUE] >= la_place[pDphsettar->phonex])
				bdur = 0;
		}
	...
	{
		switch (la_place[pDphsettar->phcur & PVALUE])
		{
			case FDENTAL:
			case FLABIAL:
	...
	if ((feacur & FVBAR) IS_PLUS)
	{
		...
		if (pDphsettar->phcur != LAP_G
			&& (pDphsettar->phonex == LAP_I
				|| pDphsettar->phonex == LAP_E))
			PAV.pspesh = 20;  */
	}
}
 ...
 *
 * *****************************************************************/
static void la_special_rules (LPTTS_HANDLE_T phTTS, short fealas, short feacur, short feanex,
							  short struclm2, short struccur, short pholas, short struclas)
{
	short                   bdur, vot; /* MVP made as a local variable */
...
*/
		...
		if ((feanex & (FNASAL | FPLOSV)) IS_PLUS)
		{
			...
			if((feacur & FPLOSV ) IS_PLUS)
				bdur=0;
			if ((la_place[pDphsettar->phcur & PVALUE] | MASKFRONT )== (la_place[pDphsettar->phonex] | MASKFRONT ))
			{		/* yes they're both done up front */
				bdur = 0;
			}
			else if (( la_place[pDphsettar->phcur & PVALUE] )== ( la_place[pDphsettar->phonex] ))
			{		/* yes they're both done up front */
				bdur = 0;
			}
		}
		...
		for (pDphsettar->np = &PA2; pDphsettar->np <= &PAB; pDphsettar->np++)
		{							   /* All fric gains to */
				if(la_place[pDphsettar->phcur & PVALUE] & FLABIAL)
			{
				PAREAL.tspesh=closure_dur;
				PAREAB.tspesh =0;
				PTONGUEBODY.tspesh=0;
			}
			else if(la_place[pDphsettar->phcur & PVALUE] & BLADEAFFECTED )
			{
				PAREAL.tspesh=0;
				PAREAB.tspesh=closure_dur;
				PTONGUEBODY.tspesh=0;
			}
				else if(la_place[pDphsettar->phcur & PVALUE] & FVELAR )
			{
				PAREAL.tspesh=0;
				PAREAB.tspesh=0;
		 ...
		 && ((feacur & FSONOR) IS_PLUS ))) 
	{
		...
		if(la_place[pholas & PVALUE] & FLABIAL)
			vot = NF20MS;
		else if(la_place[pholas & PVALUE] & FVELAR)
			vot = NF60MS;
		else
			vot = NF40MS;				   /* Asp dur for t */
